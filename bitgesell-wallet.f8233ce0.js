...
(function() {
    ...
    function urlAppendHashParams(url1, addHash) {
        ...
        if (curHash.length > 0) return url1 + '?' + addHash;
    ...
    function openProtoUrl(url1) {
        if (!url1.match(/^(web\+)?tgb?:\/\/./)) return false;
        ...
        if (useIframe) {
            ...
            if (iframeEl !== null) iframeEl.src = url1;
            setTimeout(function() {
                if (!pageHidden) window.location = url1;
...
(function() {
    ...
    var BiometricManager = function() {
        ...
        function onBiometryInfoReceived(eventType, eventData) {
            ...
            if (initRequestState.callbacks.length > 0) for(var i = 0; i < initRequestState.callbacks.length; i++){
    ...
    WebApp.openLink = function(url1, options) {
        ...
        } else window.open(url1, '_blank');
    };
    WebApp.openTelegramLink = function(url1) {
        var a = document.createElement('A');
        a.href = url1;
        if (a.protocol != 'http:' && a.protocol != 'https:') {
            console.error('[Telegram.WebApp] Url protocol is not supported', url1);
